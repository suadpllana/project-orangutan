...
must recompute *only what genuinely needs recomputing* — and nothing else.
...
**Inputs** — input keys are `str`; values are arbitrary Python objects.
...
* `executions` — total number of query-body invocations this `Engine` object has
  performed. Starts at 0 and only ever increases (except that `load()` resets it).
* `nodes` — number of derived nodes currently held in the memo cache.
* `revision` — same value as `engine.revision`.
* `inputs` — number of input keys currently set.
...
* `ctx.input(key)` — returns the input's value, or raises `InputNotSetError`.
  **The dependency is recorded either way**, so a later `set_input` for that key
  invalidates this node.
* `ctx.call(query, *args)` — evaluates another query (bringing it up to date
...
The class of the context object is not part of the API — only these two methods
are.
...
* `set_input(key, value)` where the key **is** set: the engine compares the new
  ...
  treat the values as different). If they compare equal the call is a **no-op** —
...
* `changed_at` — the revision at which this node's outcome last actually changed;
* `verified_at` — the revision at which this node was last confirmed up to date.
...
`save` writes the engine's **entire** state — revision counter, input values and
their `changed_at` stamps, and every memoized node with its outcome,
dependencies, `changed_at` and `verified_at` — into the single file at `path`.
...
> names must behave **exactly** as the saving engine did — including
...
* **Deep graphs.** `engine.get` must handle dependency chains at least
  **20,000** levels deep — a query that `ctx.call`s the next one, 20,000 times —
...
| `randomized` | 0.15 | randomly generated graphs and edit sequences, differentially compared against a model of §3.5 |

The final score is the weighted sum. **The task counts as solved when the total
score is at least 0.85.**

A representative — but deliberately much smaller and easier — subset of the

...
These 22 tests describe the behaviour `/app` has **today**. They are the floor,
not the target: the graded suite is much larger, is not shipped in the image,
and tests the specification in `SPEC.md`. The grader runs its own pristine copy
of this file, and its pass ratio multiplies the whole score -- so keep them
green, but do not mistake them for a definition of done.

Run them with:  python -m pytest public_tests -q
"""

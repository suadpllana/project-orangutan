# Rebuild pkgsolve's resolver

`/app` holds `pkgsolve`, the dependency-resolution library behind an internal
build farm. Given a manifest of root requirements and a package registry, it
picks one version of every package that has to be installed.

The version model, the range parser and the registry client all work. The
resolver does not. `pkgsolve.resolve()` currently enumerates the entire
reachable universe and returns the first assignment that satisfies everything.
It is correct on toy inputs and useless on real ones: it asks the registry for
every version of every package it can reach, then asks again while it checks
each candidate assignment, so a manifest that pulls in a few hundred versions
costs hundreds of thousands of registry round trips. And when nothing resolves
it raises `Unsolvable` with an empty `causes` set — the build farm's users get
"no solution" and no reason.

Your job is to rebuild the resolver so that `pkgsolve` satisfies the
specification below.

```
/app
├── pkgsolve/                <- the package you must rebuild
│   ├── __init__.py          public exports
│   ├── errors.py            exception hierarchy - stable, do not rename
│   ├── versions.py          Version - complete, leave it alone
│   ├── ranges.py            Range - complete, leave it alone
│   ├── requirements.py      Requirement, RootRequirement, DependencyFact
│   ├── registry.py          Registry protocol + InMemoryRegistry
│   └── solver.py            resolve() - the naive enumerator you must replace
├── public_tests/            <- a small visible subset of the checks
│   └── test_public.py       22 tests covering what already works
└── SPEC.md                  <- the same specification you are reading now
```

Run the visible tests with `python -m pytest public_tests -q` from `/app`. They
must keep passing: the graded suite runs its own pristine copy of them, and its
pass ratio **multiplies your whole score**, so breaking one costs you across
every category.
...
much larger and is not shipped in the image. Use the specification, not the
visible tests, as your definition of done.

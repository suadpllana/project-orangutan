...
``pkgsolve/**/*.py`` into a scratch tree it owns, drops its own pristine test
files next to them, and runs each category as a separate pytest process with
its own wall-clock limit, so a hang or a crash costs one category rather than
the whole run.
"""
...
# categories that explain *why* a submission failed have already run.
#
# The three laws of section 3.2, 4 and 5 -- `preference`, `explanations` and
# `budget` -- carry 0.69 between them, and the pass threshold is 0.92, so
# abandoning any one of them fails however good the rest is. Two resolvers that
# were measured doing exactly that: a correct search with no pruning scores
# 0.886, and a fast search that decides packages in the wrong order scores
# 0.900. Neither has done the task.
#
...
CATEGORIES = [
    ...
    ("api", "test_api.py", 0.07, 90),
    ("structure", "test_structure.py", 0.06, 90),
    ("preference", "test_preference.py", 0.24, 120),
    ("explanations", "test_explanations.py", 0.25, 150),
    ("budget", "test_budget.py", 0.20, 240),
    ("determinism", "test_determinism.py", 0.04, 150),
    ("randomized", "test_randomized.py", 0.14, 300),
]
MULTIPLIER_CATEGORIES = {"regression"}

SUPPORT_FILES = ["_support.py", "_oracle.py", "determinism_child.py"]
TEST_FILES = [test_file for _n, test_file, _w, _t in CATEGORIES] + SUPPORT_FILES
...
# instead, so a partial score always reaches the report. The per-category caps
# are allowed to sum above this; that is what the deadline is for.
DEFAULT_DEADLINE = 660.0

# Identifiers that only appear when an implementation is trying to notice it is
# being tested, to reach around the registry it was handed, or to reach into the
# grader's own modules. Comments and string literals are excluded before
# matching, so documentation is free.
FORBIDDEN_NAMES = {
    # the test runner
    "pytest",
    "unittest",
    "PYTEST_CURRENT_TEST",
    # the grader's own modules
    "_support",
    "_oracle",
    "determinism_child",
    # the call stack and the interpreter
    "_getframe",
    "currentframe",
    "stack",
    "extract_stack",
    ...
    "meta_path",
    "atexit",
    # reaching around the registry object it was handed
    "__closure__",
    "__globals__",
    "__dict__",
    "cell_contents",
...
# Words that are ordinary in other contexts; require the attribute owner too.
QUALIFIED_ONLY = {
...
def collect_package(submission: Path, tree: Path) -> List[Path]:
    source = submission / "pkgsolve"
    if not source.is_dir():
        raise Integrity("submission has no pkgsolve/ package")

    target = tree / "pkgsolve"
    ...
    if not (target / "__init__.py").exists():
        raise Integrity("pkgsolve/__init__.py is missing")
    return collected


def code_identifiers(path: Path):
    """Yield (name, owner, line) for every NAME token outside comments/strings."""
    ...
    for relative in files:
        ...
        for name, owner, line in code_identifiers(path):
            ...
            if allowed_owners is not None and owner not in allowed_owners:
                continue
            raise Integrity(
                f"pkgsolve/{relative}:{line} references {name!r}; the resolver "
                f"must not inspect the test runner, the environment, the call "
                f"stack, or reach around the registry it was handed"
            )
...
# running the tests
# ----------------------------------------------------------------------
def run_category(tree: Path, test_file: str, timeout: float) -> Dict:
    junit = tree / f"junit-{test_file}.xml"
    env = dict(os.environ)
    env["PYTHONPATH"] = str(tree)
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    # Fixed for the graded categories so a run is reproducible; the determinism
    # category overrides it per subprocess on purpose.
    ...
    for case in root.iter("testcase"):
        ...
        if not bad:
            ...
            # legitimately gives them nothing to look at.
            result["passed"] += 1
            continue
...
def reward_dirs() -> List[Path]:
    ...
    Where the harness mounts the sealed suite is not knowable from inside the
    bundle, so write to all of them. They are cheap.
    """
    candidates = [
...
def grade(submission: Path, deadline_seconds: float = DEFAULT_DEADLINE) -> Dict:
    report = {
        "task": "pkgsolve-resolver-explanations",
        "binary_pass": False,
        "score": 0.0,
    ...
    tree = Path(tempfile.mkdtemp(prefix="pkgsolve-grade-"))
    try:
        try:
        ...
        for name, test_file, weight, timeout in CATEGORIES:
            ...
            if remaining <= 5:
                ...
                if name in MULTIPLIER_CATEGORIES:
                    multiplier = 0.0
                print(f"  {name:<13} skipped - out of verifier time", flush=True)
                continue
            ...
            if ratio < 1.0:
                all_green = False
            print(
                f"  {name:<13} {outcome['passed']:>3}/{outcome['total']:<3} "
...
def main() -> int:
    ...
    try:
        ...
        traceback.print_exc()
        publish_reward(0.0, {"score": 0.0, "passed": False, "grader_error": repr(exc)})
        print()
        print("=" * 68)
        print("SCORE: 0.0000")
